...
- `internal/telegram` is the Bot API adapter (`mymmrac/telego`, Bot API 10.3).
  ...
  - Chat answers (`chat.go`) stream through `sendRichMessageDraft` with a stop button in private chats (`stopped_message_generation` cancels the context) and through one message edited at most every 1.5 s in groups. The bot's answers are ingested with `is_bot` so it sees its side of the talk. Dialog answers are never ingested: they may be API keys.
