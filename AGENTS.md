...
- `internal/telegram` is the Bot API adapter (`mymmrac/telego`, Bot API 10.3).
  ...
  - Chat answers (`chat.go`): in private chats a «💭 Думаю…» message with a «Остановить» button (`stop:<id>` cancels the context) shows retries and tool runs, then is deleted and the whole answer is sent; in groups one message is edited at most every 1.5 s. The bot's answers are ingested with `is_bot` so it sees its side of the talk. Dialog answers are never ingested: they may be API keys.
